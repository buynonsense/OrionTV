...
const mockedPlatform = Platform as typeof Platform;

const createWindowSize = (width: number, height: number): ScaledSize => ({
  width,
  height,
  scale: 2,
  fontScale: 1,
...
describe("DeviceUtils", () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockedPlatform.isTV = false;
  });

  describe("getDeviceType", () => {
    it("应该在真实 TV 设备上返回 tv", () => {
      mockedPlatform.isTV = true;
      mockedDimensions.get.mockReturnValue(createWindowSize(1024, 768));
      expect(DeviceUtils.getDeviceType()).toBe("tv");
    });

    it("应该在大屏非 TV 设备上返回 tablet", () => {
      mockedDimensions.get.mockReturnValue(createWindowSize(1366, 1024));
      expect(DeviceUtils.getDeviceType()).toBe("tablet");
    });

    it("应该在宽度 >= 768 时返回 tablet", () => {
      mockedDimensions.get.mockReturnValue(createWindowSize(768, 1024));
      expect(DeviceUtils.getDeviceType()).toBe("tablet");
    });

    it("应该在宽度 < 768 时返回 mobile", () => {
      mockedDimensions.get.mockReturnValue(createWindowSize(375, 812));
  ...
  describe("isTV", () => {
    it("应该在 TV 设备上返回 true", () => {
      mockedPlatform.isTV = true;
      mockedDimensions.get.mockReturnValue(createWindowSize(1920, 1080));
  ...
  describe("isLandscape", () => {
    it("应该在横屏模式下返回 true", () => {
      mockedDimensions.get.mockReturnValue(createWindowSize(812, 375));
    ...
    it("应该在宽高相等时返回 false", () => {
      mockedDimensions.get.mockReturnValue(createWindowSize(500, 500));
      expect(DeviceUtils.isLandscape()).toBe(false);
    });
  });
